...
// This will be run differently
EOF
Write-Host "Usando node para escribir archivos..." -ForegroundColor Yellow
...
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, BarChart, Bar, RadarChart, Radar, PolarGrid, PolarAngleAxis } from "recharts"
...
function TPScoreCard({ accountId, onClick }) {
  ...
  const tendIcon = tp.tendencia === "alza" ? "+" : tp.tendencia === "baja" ? "-" : "="
  return (
    <div className="metric-card" onClick={onClick} style={{ borderLeft: "3px solid " + color, cursor: onClick ? "pointer" : "default" }}
      ...
      <div style={{ marginTop: 6, fontSize: 11, fontWeight: 700, color: tp.tendencia === "alza" ? "var(--success)" : tp.tendencia === "baja" ? "var(--critical)" : "var(--silver)" }}>
        {tendIcon} Tendencia: {tp.tendencia}
      </div>
    </div>
  )
}

function MarketShareMini({ accountId }) {
  const ms = marketShare[accountId]
  if (!ms) return null
  const cats = Object.entries(ms.categorias).slice(0, 3)
  return (
    <div className="card" style={{ marginBottom: 12 }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: "var(--silver)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>Market Share por Categoria</div>
      {cats.map(([cat, data]) => {
        const genomma = data.marcas.filter(m => !m.es_competidor)
        return (
          <div key={cat} style={{ marginBottom: 10 }}>
            <div style={{ fontSize: 11, fontWeight: 700, marginBottom: 4, color: "var(--obsidian)" }}>{cat}</div>
            {genomma.map(m => (
              <div key={m.marca} style={{ marginBottom: 4 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
                  <span style={{ fontSize: 11, color: "var(--silver)" }}>{m.marca}</span>
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    <span style={{ fontSize: 11, fontWeight: 700, color: m.tendencia === "alza" ? "var(--success)" : m.tendencia === "baja" ? "var(--critical)" : "var(--silver)", fontFamily: "var(--font-mono)" }}>{m.som_pct.toFixed(1)}%</span>
                    <span style={{ fontSize: 10, color: m.tendencia === "alza" ? "var(--success)" : "var(--critical)" }}>{m.tendencia === "alza" ? "+" : m.tendencia === "baja" ? "-" : "="}{Math.abs(m.som_pct - m.som_anterior_pct).toFixed(1)}pts</span>
                  </div>
                </div>
                <div style={{ height: 4, background: "#F0F2F5", borderRadius: 2 }}>
                  <div style={{ height: 4, borderRadius: 2, width: m.som_pct + "%", background: m.tendencia === "alza" ? "var(--success)" : m.tendencia === "baja" ? "var(--critical)" : "var(--silver)", transition: "width 0.3s" }} />
                </div>
              </div>
            ))}
          </div>
...
function KBDStatusMini({ accountId }) {
  const allKbds = []
  const marcas = (brandMap[accountId] || []).map(m => m.marca)
  marcas.forEach(marca => {
    const kbdData = kbd[marca]
    if (kbdData) {
      kbdData.kbds.forEach(k => {
        if (k.status === "off_track") allKbds.push({ ...k, marca })
      })
    }
  })
  if (allKbds.length === 0) return null
  return (
    <div className="card" style={{ marginBottom: 12 }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: "var(--silver)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>KBDs Off-Track - Accion Requerida</div>
      {allKbds.slice(0, 4).map((k, i) => (
        <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: i < 3 ? "1px solid #F0F2F5" : "none" }}>
          <div>
            <span style={{ fontSize: 11, fontWeight: 700, color: "var(--obsidian)" }}>{k.marca}</span>
            <span style={{ fontSize: 10, color: "var(--silver)", marginLeft: 6 }}>{k.area}</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--critical)" }}>{k.cumplimiento_pct}%</span>
            <span style={{ fontSize: 9, background: "var(--critical-light)", color: "var(--critical)", padding: "2px 6px", borderRadius: 99, fontWeight: 700 }}>OFF TRACK</span>
          </div>
        </div>
      ))}
    </div>
...
function AgendaKAM({ accountId }) {
  ...
  if (tp && tp.score_general_pct < 70) {
    const criticos = tp.indicadores.filter(i => i.status === "off_track" && i.marcas_criticas.length > 0)
    criticos.forEach(ind => {
      ejecutar.push({
        tipo: "Tienda Perfecta",
        accion: ind.nombre + " - Marcas criticas: " + ind.marcas_criticas.join(", "),
        impacto: "TP Score actual " + tp.score_general_pct.toFixed(0) + "% vs objetivo " + tp.objetivo_pct + "%",
        urgencia: "alta"
      })
    })
  }
  ...
  if (ms) {
    Object.entries(ms.categorias).forEach(([cat, data]) => {
      data.marcas.filter(m => !m.es_competidor && m.tendencia === "baja").forEach(m => {
        negociar.push({
          tipo: "Market Share",
          accion: "Defender SOM " + m.marca + " en " + cat + " - Actual " + m.som_pct + "% vs " + m.competidor + " " + m.competidor_som_pct + "%",
          impacto: "Gap de liderazgo: " + m.gap_liderazgo_pts.toFixed(1) + " pts",
          urgencia: Math.abs(m.gap_liderazgo_pts) > 10 ? "alta" : "media"
        })
      })
    })
  }
  ...
  scorecard.filter(i => i.estado === "off_track").forEach(item => {
    negociar.push({
      tipo: "Ejecucion",
      accion: item.iniciativa + " - " + item.accion,
      impacto: item.venta_perdida_mxn > 0 ? "Venta perdida: " + formatMXN(item.venta_perdida_mxn, true) + " MXN" : "Pendiente",
      urgencia: "alta"
    })
  })
  ...
  brandas.filter(m => m.categoria === "revertir").slice(0, 2).forEach(m => {
    ejecutar.push({
      tipo: "Sell-Out",
      accion: "Plan de recuperacion " + m.marca + " - Caida " + m.sell_out_trend.toFixed(1) + "% YoY",
      impacto: "Gap YTD: " + formatMXN(Math.abs(m.gap_ytd), true) + " MXN",
      urgencia: "alta"
    })
  })
    ...
    <div className="card" style={{ marginBottom: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div>
          ...
          {ejecutar.length === 0 && <div style={{ fontSize: 12, color: "var(--success)" }}>Sin acciones urgentes esta semana</div>}
        </div>
        <div>
...
function TPDetailDrillDown({ accountId, onClose }) {
  const tp = tiendaPerfecta[accountId]
  if (!tp) return null
  const color = tp.score_general_pct >= 80 ? "var(--success)" : tp.score_general_pct >= 65 ? "var(--warning)" : "var(--critical)"
  return (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            ...
            return (
              <div key={i} style={{ marginBottom: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: "var(--obsidian)" }}>{ind.nombre}</span>
                ...
                <div style={{ height: 6, background: "#F0F2F5", borderRadius: 3 }}>
                  <div style={{ height: 6, borderRadius: 3, width: (ind.resultado_pct / ind.objetivo_pct * 100) + "%", background: c, maxWidth: "100%" }} />
                </div>
        ...
        <div>
          ...
          <table className="data-table">
            <thead>
              <tr><th>Marca</th><th style={{ textAlign: "right" }}>Dispo</th><th style={{ textAlign: "right" }}>Precio</th><th style={{ textAlign: "right" }}>Exhib</th><th style={{ textAlign: "right" }}>TP</th></tr>
            </thead>
            <tbody>
...
function MarketShareDrillDown({ accountId, onClose }) {
  const ms = marketShare[accountId]
  if (!ms) return null
  return (
    <div style={{ background: "var(--white)", border: "1px solid #E2E8F0", borderRadius: "var(--radius-lg)", padding: "20px 24px", marginBottom: 20 }}>
      ...
      {Object.entries(ms.categorias).map(([cat, data]) => (
        <div key={cat} style={{ marginBottom: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 11, color: "var(--silver)" }}>Total cat: {formatMXN(data.categoria_total_mxn_semana, true)} MXN/sem</span>
          </div>
          {data.marcas.map((m, i) => {
              ...
              <div key={i} style={{ marginBottom: 6 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 2 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    {isGeomma && <span style={{ width: 8, height: 8, borderRadius: "50%", background: c, display: "inline-block" }} />}
...
function KBDDrillDown({ accountId, onClose }) {
  const marcas = (brandMap[accountId] || []).map(m => m.marca)
        ...
        <div key={marca} style={{ marginBottom: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            ...
            <div style={{ display: "flex", gap: 12 }}>
              ...
              <span style={{ fontSize: 11, color: "var(--silver)" }}>SOM: <strong style={{ color: "var(--primary)" }}>+{data.objetivo_som_pts}pts</strong></span>
            </div>
          </div>
          {data.kbds.map((k, i) => {
            ...
            return (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 0", borderBottom: "1px solid #F0F2F5" }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, color: "var(--obsidian)", marginBottom: 2 }}><strong>[{k.area}]</strong> {k.descripcion.substring(0, 80)}{k.descripcion.length > 80 ? "..." : ""}</div>
...
function SellOutDrillDown({ onClose }) {
  ...
  const soWeekly = sellout["wmt"].weeks.map((week, i) => {
    ...
    accounts.forEach(acc => {
      entry[acc.name.split(" ")[0]] = Math.round(Object.values(sellout[acc.id].skus).reduce((s,w)=>s+w[i],0)/1000)
    })
    return entry
  })
  return (
    <div style={{ background: "var(--white)", border: "1px solid #E2E8F0", borderRadius: "var(--radius-lg)", padding: "20px 24px", marginBottom: 20 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <div style={{ fontSize: 15, fontWeight: 800 }}>Sell-Out - Tendencia Semanal por Cuenta</div>
        <button className="btn btn-secondary" onClick={onClose} style={{ fontSize: 12, padding: "6px 14px" }}>Cerrar</button>
      </div>
      <ResponsiveContainer width="100%" height={200}>
        <LineChart data={soWeekly}>
          ...
          {accounts.map((acc, i) => (
            <Line key={acc.id} type="monotone" dataKey={acc.name.split(" ")[0]} stroke={SKU_COLORS[i]} strokeWidth={2} dot={false} />
          ))}
        </LineChart>
      </ResponsiveContainer>
      <div style={{ marginTop: 16, fontSize: 12, fontWeight: 700, color: "var(--silver)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 10 }}>Ultima semana vs anterior (miles MXN)</div>
...
function BrandCard({ marca }) {
  ...
  return (
    <div style={{ background: "var(--white)", border: "1px solid #E2E8F0", borderLeft: "4px solid " + cfg.color, borderRadius: "var(--radius-md)", padding: "10px 12px", marginBottom: 8 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <span style={{ fontSize: 11, color: "var(--silver)" }}>{marca.inventario_dias} dias inv.</span>
        <span style={{ fontSize: 11, color: "var(--silver)" }}>SOM: {marca.market_share}%</span>
  ...
  return (
    <div className="account-card" onClick={() => onSelect(account.id)} style={{ borderColor: isOxxo ? "var(--critical)" : undefined }}>
      ...
      <div className="account-stats">
        <div><div className="account-stat-label">Sell-Out Sem. (MXN)</div><div className="account-stat-value">{formatMXN(lastWeekSO, true)}</div></div>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div>
            {summary.gap_alert && (
              <div className={"alert " + (summary.gap_alert === "quiebre" ? "alert-red" : "alert-amber")} style={{ marginBottom: 12 }}>
                <span>!</span>
                <div>
                  <strong>{summary.gap_alert === "quiebre" ? "Alerta: Riesgo de quiebre de inventario" : "Alerta: Posible sobreinventario"}</strong>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, marginBottom: 8 }}>
              ...
              <AlertCard label="Cobertura Promedio" value={summary.avgCov.toFixed(0) + " dias"} sub="Minimo sano: 14 dias" color="var(--info)" onClick={() => toggleDrillDown("cobertura")} />
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, marginBottom: 8 }}>
              ...
              <KPICard label="Sell-In Mes (MXN)" value={formatMXN(summary.si_real_mes, true)} target={formatMXN(summary.si_obj_mes, true)} vsLY={summary.si_vs_ly} pct={(summary.si_real_mes / summary.si_obj_mes) * 100} onClick={() => toggleDrillDown("sellin")} />
              <KPICard label="Sell-In Trimestre (MXN)" value={formatMXN(summary.si_real_trim, true)} target={formatMXN(summary.si_obj_trim, true)} vsLY={summary.si_vs_ly} pct={(summary.si_real_trim / summary.si_obj_trim) * 100} onClick={() => toggleDrillDown("sellin")} />
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 8 }}>
              ...
              <AlertCard label="% Gasto Fondos" value={summary.fondosExec.toFixed(0) + "%"} sub="Prom. todas las cuentas" color={summary.fondosExec < 60 ? "var(--critical)" : summary.fondosExec < 75 ? "var(--warning)" : "var(--success)"} onClick={() => toggleDrillDown("fondos")} />
            ...
            {drillDown === "alertas" && <div style={{ background: "var(--white)", border: "1px solid #E2E8F0", borderRadius: "var(--radius-lg)", padding: "20px 24px", marginBottom: 16 }}><div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}><div style={{ fontSize: 15, fontWeight: 800 }}>Alertas Off-Track - Todas las Cuentas</div><button className="btn btn-secondary" onClick={() => setDrillDown(null)} style={{ fontSize: 12 }}>Cerrar</button></div>{accounts.map(acc => { const items = (executionScorecard[acc.id] || []).filter(i => i.estado === "off_track"); return items.map((item, i) => <div key={acc.id+i} style={{ background: "var(--critical-light)", borderRadius: "var(--radius-md)", padding: "8px 12px", marginBottom: 6, borderLeft: "3px solid var(--critical)" }}><div style={{ display: "flex", justifyContent: "space-between" }}><div><strong>{item.iniciativa}</strong><span style={{ fontSize: 11, color: "var(--silver)", marginLeft: 8 }}>- {acc.name}</span></div><span style={{ fontSize: 11, fontWeight: 700, color: "var(--critical)", fontFamily: "var(--font-mono)" }}>{item.venta_perdida_mxn > 0 ? "-" + formatMXN(item.venta_perdida_mxn, true) + " MXN" : "Pendiente"}</span></div></div>) })}</div>}
            {drillDown === "tp" && <TPDetailDrillDown accountId={selectedAccount} onClose={() => setDrillDown(null)} />}
            {drillDown === "kbd" && <KBDDrillDown accountId={selectedAccount} onClose={() => setDrillDown(null)} />}
        ...
        {activeSection === "agenda" && (
          <div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
              {accounts.map(acc => (
                <button key={acc.id} onClick={() => setSelectedAccount(acc.id)} className={"btn " + (selectedAccount === acc.id ? "btn-primary" : "btn-secondary")} style={{ fontSize: 12, padding: "6px 14px" }}>{acc.name}</button>
              ))}
            </div>
            <AgendaKAM accountId={selectedAccount} />
            <MarketShareMini accountId={selectedAccount} />
            <KBDStatusMini accountId={selectedAccount} />
          </div>
        )}
          ...
          <div>
            ...
            {account && (
              <div>
                ...
                {drillDown === "tp_detail" && <TPDetailDrillDown accountId={selectedAccount} onClose={() => setDrillDown(null)} />}
                <div className="card">
          ...
          <div>
            ...
            <div className="card">
              <div className="section-header" style={{ marginBottom: 14 }}>
                <div>
                  <div className="section-title">Mapa de Prioridades por Marca - {account && account.name}</div>
                  <div className="section-sub">Incluye sell-out YoY, inventario, market share y oportunidad en MXN</div>
                </div>
              </div>
        ...
        {activeSection === "share" && (
          <div>
            ...
            <div className="card">
              <div className="section-title" style={{ marginBottom: 14 }}>Market Share por Categoria vs Competencia - {account && account.name}</div>
              <MarketShareDrillDown accountId={selectedAccount} onClose={() => {}} />
            </div>
          </div>
        )}
        ...
        {activeSection === "tp" && (
          <div>
            ...
            <TPDetailDrillDown accountId={selectedAccount} onClose={() => {}} />
          </div>
        )}
        ...
        {activeSection === "kbd" && (
          <div>
            ...
            <KBDDrillDown accountId={selectedAccount} onClose={() => {}} />
          </div>
        )}
...
fs.writeFileSync(base + "/src/pages/Dashboard.jsx", dashboard, "utf8")
console.log("Dashboard completo OK")
'@
